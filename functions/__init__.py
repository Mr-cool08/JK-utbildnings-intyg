# Copyright (c) Liam Suorsa and Mika Suorsa
"""Aggregate module for application helper functions."""

from __future__ import annotations

import importlib

from sqlalchemy import create_engine

from functions.admin import log_admin_action
from functions.applications import (
    _clean_optional_text,
    _ensure_company,
    approve_application_request,
    create_application_request,
    get_application_request,
    list_application_requests,
    list_companies_for_invoicing,
    reject_application_request,
)
from functions.database import (
    APP_ROOT,
    TABLE_REGISTRY,
    _build_engine,
    _is_truthy,
    admin_audit_log_table,
    application_requests_table,
    companies_table,
    company_users_table,
    create_database,
    get_engine,
    metadata,
    organization_link_requests_table,
    password_resets_table,
    pending_supervisors_table,
    pending_users_table,
    reset_engine,
    schema_migrations_table,
    supervisor_password_resets_table,
    supervisor_connections_table,
    supervisor_link_requests_table,
    supervisors_table,
    user_pdfs_table,
    users_table,
)
from functions.demo import (
    DEMO_PDF_DEFINITIONS,
    create_test_user,
    ensure_demo_data,
    reset_demo_database,
)
from functions.hashing import (
    SALT,
    _is_valid_hash,
    email_lookup_values,
    hash_password,
    hash_value,
    normalize_email,
    normalize_email_reference,
    normalize_orgnr,
    normalize_personnummer,
    validate_orgnr,
    verify_password,
)
from functions.logging import configure_module_logger, mask_email_reference, mask_hash
from functions.organization_links import (
    approve_organization_link_request,
    delete_organization_link_requests_for_user,
    get_account_orgnr,
    get_public_organization_overview,
    list_pending_organization_link_requests,
    register_standard_account,
    reject_organization_link_request,
    update_organization_request_contact_details,
)
from functions.password_resets import (
    create_password_reset_token,
    create_supervisor_password_reset_token,
    get_password_reset,
    get_supervisor_password_reset,
    reset_password_with_token,
    reset_supervisor_password_with_token,
)
from functions.pdf_storage import (
    count_user_pdfs,
    delete_user_pdf,
    get_pdf_content,
    get_pdf_metadata,
    get_user_pdfs,
    store_pdf_blob,
    update_pdf_categories,
)
from functions.supervisors import (
    admin_create_supervisor,
    admin_delete_supervisor_account,
    admin_link_supervisor_to_user,
    check_pending_supervisor_hash,
    create_supervisor_link_request,
    get_supervisor_email_hash,
    get_supervisor_login_details_for_orgnr,
    get_supervisor_name_by_hash,
    get_supervisor_overview,
    list_supervisor_connections,
    list_user_link_requests,
    list_user_supervisor_connections,
    supervisor_activate_account,
    supervisor_exists,
    supervisor_has_access,
    supervisor_remove_connection,
    user_accept_link_request,
    user_reject_link_request,
    user_remove_supervisor_connection,
    user_remove_supervisor_connection_by_id,
    verify_supervisor_credentials,
)
from functions.table_admin import (
    create_table_row,
    delete_table_row,
    fetch_table_rows,
    get_table_schema,
    update_table_row,
)
from functions.users import (
    admin_delete_user_account,
    admin_delete_user_account_by_hash,
    admin_create_user,
    admin_update_user_account,
    check_password_user,
    check_pending_user,
    check_pending_user_hash,
    check_personnummer_password,
    check_user_exists,
    complete_legacy_email_reference,
    get_user_info,
    get_admin_password_status,
    get_username,
    get_username_by_personnummer_hash,
    get_pending_user_personnummer_hash,
    list_admin_accounts,
    list_legacy_email_references,
    user_create_user,
    verify_certificate,
)


logger = configure_module_logger(__name__)

__all__ = [
    "APP_ROOT",
    "DEMO_PDF_DEFINITIONS",
    "SALT",
    "TABLE_REGISTRY",
    "_build_engine",
    "_clean_optional_text",
    "_ensure_company",
    "_is_truthy",
    "admin_audit_log_table",
    "admin_create_supervisor",
    "admin_delete_supervisor_account",
    "admin_create_user",
    "admin_delete_user_account",
    "admin_delete_user_account_by_hash",
    "admin_update_user_account",
    "admin_link_supervisor_to_user",
    "application_requests_table",
    "approve_application_request",
    "check_password_user",
    "check_pending_supervisor_hash",
    "check_pending_user",
    "check_pending_user_hash",
    "check_personnummer_password",
    "check_user_exists",
    "complete_legacy_email_reference",
    "companies_table",
    "company_users_table",
    "create_application_request",
    "create_database",
    "create_password_reset_token",
    "create_supervisor_password_reset_token",
    "create_engine",
    "create_supervisor_link_request",
    "create_table_row",
    "create_test_user",
    "count_user_pdfs",
    "delete_table_row",
    "delete_user_pdf",
    "email_lookup_values",
    "ensure_demo_data",
    "fetch_table_rows",
    "get_application_request",
    "get_engine",
    "get_password_reset",
    "get_pdf_content",
    "get_pdf_metadata",
    "get_supervisor_email_hash",
    "get_supervisor_login_details_for_orgnr",
    "get_supervisor_name_by_hash",
    "get_supervisor_overview",
    "get_supervisor_password_reset",
    "get_table_schema",
    "get_user_info",
    "get_admin_password_status",
    "get_account_orgnr",
    "get_public_organization_overview",
    "get_user_pdfs",
    "get_username",
    "get_username_by_personnummer_hash",
    "get_pending_user_personnummer_hash",
    "hash_password",
    "hash_value",
    "list_application_requests",
    "list_admin_accounts",
    "list_companies_for_invoicing",
    "list_legacy_email_references",
    "list_pending_organization_link_requests",
    "list_supervisor_connections",
    "list_user_link_requests",
    "list_user_supervisor_connections",
    "log_admin_action",
    "logger",
    "mask_hash",
    "mask_email_reference",
    "metadata",
    "normalize_email",
    "normalize_email_reference",
    "normalize_orgnr",
    "normalize_personnummer",
    "organization_link_requests_table",
    "password_resets_table",
    "pending_supervisors_table",
    "pending_users_table",
    "register_standard_account",
    "reject_application_request",
    "reject_organization_link_request",
    "reset_demo_database",
    "reset_engine",
    "reset_password_with_token",
    "reset_supervisor_password_with_token",
    "schema_migrations_table",
    "store_pdf_blob",
    "supervisor_activate_account",
    "supervisor_connections_table",
    "supervisor_exists",
    "supervisor_has_access",
    "supervisor_password_resets_table",
    "supervisor_link_requests_table",
    "supervisor_remove_connection",
    "approve_organization_link_request",
    "delete_organization_link_requests_for_user",
    "supervisors_table",
    "update_pdf_categories",
    "update_organization_request_contact_details",
    "update_table_row",
    "user_accept_link_request",
    "user_create_user",
    "user_pdfs_table",
    "user_reject_link_request",
    "user_remove_supervisor_connection",
    "user_remove_supervisor_connection_by_id",
    "users_table",
    "validate_orgnr",
    "verify_certificate",
    "verify_password",
    "verify_supervisor_credentials",
]
