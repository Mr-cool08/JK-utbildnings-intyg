...
from functions.security.hashing import hash_value, normalize_email, validate_orgnr
...
def create_application_request(
    ...
    logger.info("Mottog kontoansökan %s för %s", request_id, mask_hash(email_hash))
...
def approve_application_request(application_id: int, reviewer: str) -> Dict[str, Any]:
    ...
    with get_engine().begin() as conn:
        ...
        if stored_orgnr:
            ...
            company_id, created, company_display = ensure_company(
                conn,
                validated_orgnr,
