...
from functions.db import get_engine, user_pdfs_table
...
def store_pdf_blob(
    ...
    logger.info("Stored PDF %s for %s as id %s", filename, mask_hash(personnummer_hash), pdf_id)
    return int(pdf_id)
...
def _import_legacy_pdfs(personnummer_hash: str, existing_filenames: Set[str]) -> bool:
    ...
    for entry in sorted(os.listdir(legacy_dir)):
        ...
        try:
            ...
            logger.exception("Failed to read legacy PDF %s for %s", path, personnummer_hash)
            continue

        try:
            store_pdf_blob(personnummer_hash, entry, content)
        except Exception:  # pragma: no cover - defensive; store_pdf_blob rarely fails
            logger.exception("Failed to import legacy PDF %s for %s", entry, personnummer_hash)
            continue
...
def get_pdf_content(personnummer_hash: str, pdf_id: int) -> Optional[Tuple[str, bytes]]:
    ...
    with get_engine().connect() as conn:
        row = conn.execute(
            select(user_pdfs_table.c.filename, user_pdfs_table.c.content).where(
                user_pdfs_table.c.personnummer == personnummer_hash,
                user_pdfs_table.c.id == pdf_id,
            )
        ).first()
