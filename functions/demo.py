...
from functions.companies import ensure_company
from functions.constants import APP_ROOT
...
from functions.pdf.storage import _serialize_categories, store_pdf_blob
...
def ensure_demo_data(
    *,
    ...
    if normalized_orgnr:
        with engine.begin() as conn:
            try:
                company_id, created_company, company_name = ensure_company(
                    conn,
                    normalized_orgnr,
                    supervisor_name,
...
def reset_demo_database(demo_defaults: Dict[str, str]) -> bool:
    ...
    from functions.db import reset_engine, create_database
