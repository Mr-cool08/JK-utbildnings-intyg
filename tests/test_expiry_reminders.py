...
def _create_active_user_with_raw_email(
    empty_db,
    *,
    personnummer: str,
    email: str,
...
def _create_company_account_with_raw_email(
    empty_db,
    *,
    email: str,
    company_name: str,
...
def _capture_error_logs(monkeypatch):
    errors = []
...
def test_expiry_reminders_report_invalid_private_email_reference(empty_db, monkeypatch):
    _set_reminder_config(monkeypatch, enabled=True)
    private_calls, supervisor_calls = _capture_email_calls(monkeypatch)
    ...
    assert result.unresolved_private_recipients == 1
    assert private_calls == []
    assert supervisor_calls == []
    assert len(errors) == 1
...
def test_expiry_reminders_report_invalid_supervisor_email_reference(
    ...
    personnummer_hash = _create_active_user(
        empty_db,
        personnummer="19901111-1234",
        email="giltig@example.com",
        username="Gillis",
    )
    supervisor_reference = _create_company_account_with_raw_email(
        empty_db,
        email="bolag@invalid",
    ...
    assert result.unresolved_supervisor_recipients == 1
    assert len(private_calls) == 1
    assert supervisor_calls == []
    assert len(errors) == 1
    assert "Ogiltiga e-postadresser upptäcktes i utgångspåminnelsejobbet" in errors[0]
    assert "företagskonto" in errors[0]
